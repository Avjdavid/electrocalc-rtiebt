...
const Card = ({
  ...
  <div
    className={`bg-white rounded-xl border border-zinc-200 shadow-sm p-5 ${className}`}
  >
    {children}
  </div>
);
...
const Input = (p: React.InputHTMLAttributes<HTMLInputElement>) => (
  <input
    {...p}
    className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-zinc-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
  />
);
...
const Select = (p: React.SelectHTMLAttributes<HTMLSelectElement>) => (
  <select
    {...p}
    className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-zinc-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-white transition-all"
  />
);
...
export default function App() {
  ...
  const [tDevice, setTDevice] = useState<number>(0.2); // s (tempo de corte máx do disjuntor)
  ...
  const resByBreaker = useMemo(() => {
    ...
    const Iz_base =
      TABLE.find((r) => r.s === finalS)?.Iz ?? TABLE[TABLE.length - 1].Iz;
  ...
  const resByCable = useMemo(() => {
    ...
    const dropAtBreaker =
      safeBreaker > 0
        ? round(
            voltageDropPercent(mode, safeBreaker, lenB, cableS, V),
            2
          )
  ...
  const resByPower = useMemo(() => {
    const denom =
      mode === "1F" ? V * cosPhi : Math.sqrt(3) * 400 * cosPhi;
    ...
    const finalS = Math.max(base.s, best.s);
    const Iz_corr =
      (TABLE.find((r) => r.s === finalS)?.Iz ??
        TABLE[TABLE.length - 1].Iz) * kTotal;
  ...
  // === Curto-circuito (adiabático) p/ secção escolhida ===
  ...
  return (
    <div className="min-h-screen bg-zinc-50 text-zinc-900 pb-10 font-sans">
      ...
      <header className="bg-white border-b border-zinc-200 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link
              to="/tabela"
              className="text-xs rounded-md border px-2 py-1 hover:bg-zinc-100"
            >
              Tabela de Secções
            </Link>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-6 gap-3">
            ...
            <div>
              ...
              <Input
                ...
                max="9"
                value={nCircuits}
                onChange={(e) => setNCircuits(Number(e.target.value))}
              />
            </div>
            <div>
              ...
              <Input
                ...
                max="10"
                value={dropLimit}
                onChange={(e) => setDropLimit(Number(e.target.value))}
              />
            </div>
            <div className="col-span-2">
              <div className="text-xs text-zinc-600 bg-white rounded-md border px-2 py-2">
                <div>
                  <b>kT</b>={kT} · <b>kG</b>={kG} → <b>kTotal</b>= {kTotal}
                </div>
                <div className="text-[10px]">
                  Iz_corr = Iz_quadro × kT × kG
                </div>
              </div>
            </div>
          </div>
        ...
        <Card>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-3 bg-zinc-50 rounded-lg text-center">
              ...
              <div className="text-3xl font-black text-green-700">
                {resByBreaker.finalS} mm²
              </div>
            </div>
            <div className="p-3 bg-zinc-50 rounded-lg">
              <ResultRow
                label="Iz (corrigida)"
                value={`${resByBreaker.Iz_corr} A`}
              />
              <ResultRow
                label="Queda estimada"
                value={`${resByBreaker.finalDrop}%`}
                status={
                  resByBreaker.finalDrop > dropLimit ? "error" : "ok"
                }
              />
            </div>
          </div>
          <div className="mt-2">
            <ResultRow
              ...
              value={resByBreaker.coord.ok1 ? "OK" : "NOK"}
              status={resByBreaker.coord.ok1 ? "ok" : "error"}
            />
            <ResultRow
              label="I₂ ≤ 1,45·Iz"
              value={resByBreaker.coord.ok2 ? "OK" : "NOK"}
              status={resByBreaker.coord.ok2 ? "ok" : "error"}
              sub={`I₂≈${round(resByBreaker.coord.I2)} A`}
            />
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            <div className="p-3 bg-green-50 rounded-lg">
              ...
              <span className="text-xl font-bold text-green-800">
                {resByCable.safeBreaker > 0
                  ? `${resByCable.safeBreaker} A`
                  : "—"}
              </span>
            ...
            <div className="p-3 bg-zinc-50 rounded-lg">
              <ResultRow
                label="Iz (corrigida)"
                value={`${resByCable.Iz_corr} A`}
              />
              <ResultRow
                label="Queda @disjuntor"
                value={`${resByCable.dropAtBreaker}%`}
                status={
                  resByCable.dropAtBreaker > dropLimit ? "error" : "ok"
                }
              />
              <ResultRow
                label="Potência disponível"
                value={`${resByCable.powerMax.toLocaleString(
                  "pt-PT"
                )} W`}
              />
            </div>
          ...
          <div className="mt-2">
            <ResultRow
              label="Coordenação (IB≤In≤Iz)"
              value={resByCable.coord.ok1 ? "OK" : "NOK"}
              status={resByCable.coord.ok1 ? "ok" : "error"}
            />
            <ResultRow
              label="I₂ ≤ 1,45·Iz"
              value={resByCable.coord.ok2 ? "OK" : "NOK"}
              status={resByCable.coord.ok2 ? "ok" : "error"}
            />
          </div>
          <div className="mt-3 p-3 rounded-lg border">
            <div className="font-semibold text-sm mb-1">
              Curto-circuito (adiabático)
            </div>
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                <Label>t disjuntor (s)</Label>
                <Input
                  type="number"
                  step="0.01"
                  value={tDevice}
                  onChange={(e) => setTDevice(Number(e.target.value))}
                />
              </div>
              <div className="flex items-end">
                <div className="w-full text-sm">
                  <div>
                    t<sub>adm</sub> do cabo: <b>{scByCable.tAdm}s</b>
                  </div>
                  <div
                    className={
                      scByCable.ok ? "text-green-700" : "text-red-600"
                    }
                  >
                    {scByCable.ok ? "OK" : "NOK"} (t
                    <sub>adm</sub> ≥ t<sub>disj</sub>)
                  </div>
                </div>
              </div>
            </div>
        ...
        <Card>
          ...
          <div className="grid grid-cols-2 gap-4 mb-3">
            ...
            <div className="col-span-2 sm:col-span-1">
              ...
              <Input
                ...
                min="1"
                value={lenC}
                onChange={(e) => setLenC(Number(e.target.value))}
              />
            </div>
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-3">
            <div className="p-3 bg-purple-50 rounded-lg">
              <div className="text-xs text-purple-700 uppercase font-semibold">
                Secção
              </div>
              <div className="text-3xl font-black text-purple-900">
                {resByPower.finalS} mm²
              </div>
            </div>
            <div className="p-3 bg-zinc-50 rounded-lg">
              <ResultRow
                label="Ib (estimada)"
                value={`${resByPower.Ib} A`}
              />
              <ResultRow
                label="Queda estimada"
                value={`${resByPower.dV}%`}
                status={
                  resByPower.dV > dropLimit ? "warn" : "ok"
                }
              />
              <ResultRow
                label="Iz (corrigida)"
                value={`${resByPower.Iz_corr} A`}
              />
              <ResultRow
                label="Coordenação (IB≤In≤Iz)"
                value={resByPower.coord.ok1 ? "OK" : "NOK"}
                status={resByPower.coord.ok1 ? "ok" : "error"}
              />
              <ResultRow
                label="I₂ ≤ 1,45·Iz"
                value={resByPower.coord.ok2 ? "OK" : "NOK"}
                status={resByPower.coord.ok2 ? "ok" : "error"}
              />
            </div>
          </div>
        </Card>
      ...
      <footer className="text-center text-xs text-zinc-400 max-w-md mx-auto px-6 pb-6">
        <p className="mb-2">
          ⚠️ Esta ferramenta aproxima valores. Valide sempre com o RTIEBT.
        </p>
        <p>
          Regras aplicadas: IB≤In≤Iz_corr, I₂≤1,45·Iz_corr, verificação
          adiabática S–I–t e queda de tensão.
        </p>
      </footer>
    </div>
  );
